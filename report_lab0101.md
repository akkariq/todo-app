...
#### 3. **Vite**
Vite — современный бьдловщик для фронтенда. Использует нативные ES-модули браузера в режиме разработки — запуск проекта занимает миллисекунды. HMR (горячая замена модулей) обновляет страницу без перезагрузки.
...
#### 5. **Tailwind CSS**
Утилитарный CSS-фреймворк: классы применяются напрямую в JSX, что ускоряет вёрстку и исключает переключение между файлами.
...
- [x] **Задача B: Переключение статуса** (обязательно)
  - Реализована `toggleTask` через `map` и spread-оператор
...
![Главный экран](Снимок%20экрана%202026-04-27%20212117.png)
*Основной экран приложения: список задач, чекбоксы, кнопки удаления и статистика.*

**Добавление новой задачи**

![Добавление задачи](Снимок%20экрана%202026-04-27%20212249.png)
*Ввод текста и нажатие «Добавить» или Enter.*

**Задача в состоянии «выполнено»**

![Выполненная задача](Снимок%20экрана%202026-04-27%20212302.png)
*Чекбокс активирован — текст зачёркнут и осерён.*

**Пустой список**

![Пустой список](Снимок%20экрана%202026-04-27%20212319.png)
*При отсутствии задач отображается информационное сообщение.*
...
**Ответ:**
`useState` — это хук, позволяющий функциональным компонентам React хранить внутреннее состояние между рендерами. Вызывается как `const [state, setState] = useState(initialValue)` и возвращает пару: текущее значение состояния и функцию его обновления. Каждый вызов сеттера (например, `setTasks(...)`) тригирует перерендеринг компонента с новым значением. В данном проекте используется два состояния: `tasks` (массив задач) и `newTask` (текст в поле ввода).
